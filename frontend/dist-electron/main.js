...
import { BrowserWindow, Menu, Notification, Tray, app, globalShortcut, ipcMain, powerMonitor } from "electron";
...
var require_utils$1 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var path$30 = __require("path");
	module.exports.checkPath = function checkPath(pth) {
		if (process.platform === "win32") {
			if (/[<>:"|?*]/.test(pth.replace(path$30.parse(pth).root, ""))) {
...
var require_stat = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		...
		util$2.callbackify(getStats)(src, dest, opts, (err, stats) => {
			...
			if (destStat) {
				if (areIdentical(srcStat, destStat)) {
					const srcBaseName = path$29.basename(src);
					const destBaseName = path$29.basename(dest);
	...
	function checkParentPaths(src, srcStat, dest, funcName, cb) {
		const srcParent = path$29.resolve(path$29.dirname(src));
		const destParent = path$29.resolve(path$29.dirname(dest));
		if (destParent === srcParent || destParent === path$29.parse(destParent).root) return cb();
	...
	function checkParentPathsSync(src, srcStat, dest, funcName) {
		const srcParent = path$29.resolve(path$29.dirname(src));
		const destParent = path$29.resolve(path$29.dirname(dest));
		if (destParent === srcParent || destParent === path$29.parse(destParent).root) return;
		let destStat;
		try {
	...
	function isSrcSubdir(src, dest) {
		const srcArr = path$29.resolve(src).split(path$29.sep).filter((i) => i);
		const destArr = path$29.resolve(dest).split(path$29.sep).filter((i) => i);
...
var require_copy$1 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function copyDirItem(items, item, src, dest, opts, cb) {
		const srcItem = path$28.join(src, item);
		const destItem = path$28.join(dest, item);
...
var require_copy_sync = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function handleFilterAndCopy(destStat, src, dest, opts) {
		...
		const destParent = path$27.dirname(dest);
	...
	function copyDirItem(item, src, dest, opts) {
		const srcItem = path$27.join(src, item);
		const destItem = path$27.join(dest, item);
		const { destStat } = stat.checkPathsSync(srcItem, destItem, "copy", opts);
		return startCopy(destStat, srcItem, destItem, opts);
	}
	function onLink(destStat, src, dest, opts) {
		let resolvedSrc = fs.readlinkSync(src);
		if (opts.dereference) resolvedSrc = path$27.resolve(process.cwd(), resolvedSrc);
		if (!destStat) return fs.symlinkSync(resolvedSrc, dest);
			...
			if (opts.dereference) resolvedDest = path$27.resolve(process.cwd(), resolvedDest);
...
var require_rimraf = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function rmkids(p, options, cb) {
		...
		options.readdir(p, (er, files) => {
			...
			files.forEach((f) => {
				rimraf(path$26.join(p, f), options, (er) => {
					if (errState) return;
	...
	function rmkidsSync(p, options) {
		assert(p);
		assert(options);
		options.readdirSync(p).forEach((f) => rimrafSync(path$26.join(p, f), options));
		if (isWindows) {
...
var require_empty = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var emptyDir = u(async function emptyDir(dir) {
		...
		return Promise.all(items.map((item) => remove.remove(path$25.join(dir, item))));
	});
...
var require_file$1 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function createFile(file, callback) {
		...
		fs.stat(file, (err, stats) => {
			...
			const dir = path$24.dirname(file);
...
var require_symlink_paths = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	*/
	function symlinkPaths(srcpath, dstpath, callback) {
		if (path$22.isAbsolute(srcpath)) return fs.lstat(srcpath, (err) => {
			if (err) {
			...
			const dstdir = path$22.dirname(dstpath);
			const relativeToDst = path$22.join(dstdir, srcpath);
			return pathExists(relativeToDst, (err, exists) => {
				...
				if (exists) return callback(null, {
					...
					return callback(null, {
						toCwd: srcpath,
						toDst: path$22.relative(dstdir, srcpath)
					});
				});
			});
		}
...
var require_output_file = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function outputFile(file, data, encoding, callback) {
		...
		const dir = path$20.dirname(file);
...
var require_move$1 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function move(src, dest, opts, cb) {
		...
		stat.checkPaths(src, dest, "move", opts, (err, stats) => {
			...
			stat.checkParentPaths(src, srcStat, dest, "move", (err) => {
				...
				mkdirp(path$19.dirname(dest), (err) => {
					if (err) return cb(err);
	...
	function isParentRoot(dest) {
		const parent = path$19.dirname(dest);
		return path$19.parse(parent).root === parent;
	}
...
var require_move_sync = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function moveSync(src, dest, opts) {
		...
		if (!isParentRoot(dest)) mkdirpSync(path$18.dirname(dest));
		return doRename(src, dest, overwrite, isChangingCase);
	}
	function isParentRoot(dest) {
		const parent = path$18.dirname(dest);
		return path$18.parse(parent).root === parent;
	}
...
var require_DownloadedUpdateHelper = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var DownloadedUpdateHelper = class {
		...
		get cacheDirForPendingUpdate() {
			return path$17.join(this.cacheDir, "pending");
		}
		...
		*/
		async getValidCachedUpdateFile(fileInfo, logger) {
			...
			const updateFile = path$17.join(this.cacheDirForPendingUpdate, cachedInfo.fileName);
		...
		getUpdateInfoFile() {
			return path$17.join(this.cacheDirForPendingUpdate, "update-info.json");
		}
	};
	...
	async function createTempUpdateFile(name, cacheDir, log) {
		let nameCounter = 0;
		let result = path$17.join(cacheDir, name);
		for (let i = 0; i < 3; i++) try {
			...
			result = path$17.join(cacheDir, `${nameCounter++}-${name}`);
		}
		return result;
	}
}));
...
var require_AppAdapter = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function getAppCacheDir() {
		...
		let result;
		if (process.platform === "win32") result = process.env["LOCALAPPDATA"] || path$16.join(homedir, "AppData", "Local");
		else if (process.platform === "darwin") result = path$16.join(homedir, "Library", "Caches");
		else result = process.env["XDG_CACHE_HOME"] || path$16.join(homedir, ".cache");
		return result;
	}
}));
//#endregion
...
var require_ElectronAppAdapter = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var ElectronAppAdapter = class {
		...
		get appUpdateConfigPath() {
			return this.isPackaged ? path$15.join(process.resourcesPath, "app-update.yml") : path$15.join(this.app.getAppPath(), "dev-app-update.yml");
		}
		get userDataPath() {
	...
	var PrivateGitHubProvider = class extends GitHubProvider_1.BaseGitHubProvider {
		...
		resolveFiles(updateInfo) {
			return (0, Provider_1.getFileList)(updateInfo).map((it) => {
				const name = path$14.posix.basename(it.url).replace(/ /g, "-");
	...
	exports.AppUpdater = class AppUpdater extends events_1.EventEmitter {
		...
		*/
		...
		async getOrCreateStagingUserId() {
			const file = path$13.join(this.app.userDataPath, ".updaterId");
			try {
		...
		async getOrCreateDownloadHelper() {
			...
			if (result == null) {
				...
				const cacheDir = path$13.join(this.app.baseCachePath, dirName || this.app.name);
		...
		async executeDownload(taskOptions) {
			...
			function getCacheUpdateFileName() {
				...
				if (urlPath.toLowerCase().endsWith(`.${taskOptions.fileExtension.toLowerCase()}`)) return path$13.basename(urlPath);
				else return path$13.basename(taskOptions.fileInfo.info.url);
			}
			const downloadedUpdateHelper = await this.getOrCreateDownloadHelper();
			const cacheDir = downloadedUpdateHelper.cacheDirForPendingUpdate;
			await (0, fs_extra_1.mkdir)(cacheDir, { recursive: true });
			const updateFileName = getCacheUpdateFileName();
			let updateFile = path$13.join(cacheDir, updateFileName);
			const packageFile = packageInfo == null ? null : path$13.join(cacheDir, `package-${version}${path$13.extname(packageInfo.path) || ".7z"}`);
			const done = async (isSaveCache) => {
				...
				const currentBlockMapFile = path$13.join(cacheDir, "current.blockmap");
				if (await (0, fs_extra_1.pathExists)(currentBlockMapFile)) await (0, fs_extra_1.copyFile)(currentBlockMapFile, path$13.join(downloadedUpdateHelper.cacheDir, "current.blockmap"));
		...
		async differentialDownloadInstaller(fileInfo, downloadUpdateOptions, installerPath, provider, oldInstallerFileName) {
			try {
				...
				const downloadOptions = {
					newUrl: fileInfo.url,
					oldFile: path$13.join(this.downloadedUpdateHelper.cacheDir, oldInstallerFileName),
					logger: this._logger,
				...
				const saveBlockMapToCacheDir = async (blockMapData, cacheDir) => {
					const blockMapFile = path$13.join(cacheDir, "current.blockmap");
	...
	var BaseUpdater = class extends AppUpdater_1.AppUpdater {
		...
		*/
		sanitizeEnvPath(envPath) {
			return envPath.split(path$12.delimiter).filter((dir) => path$12.isAbsolute(dir)).join(path$12.delimiter);
		}
...
var require_AppImageUpdater = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var AppImageUpdater = class extends BaseUpdater_1.BaseUpdater {
		...
		doInstall(options) {
			...
			if (!path$11.isAbsolute(appImageFile) || appImageFile.includes("\0")) throw (0, builder_util_runtime_1.newError)(`APPIMAGE env is not a valid absolute path: "${appImageFile}"`, "ERR_UPDATER_OLD_FILE_NOT_FOUND");
			(0, fs_1$1.unlinkSync)(appImageFile);
			let destination;
			const existingBaseName = path$11.basename(appImageFile);
			const installerPath = this.installerPath;
			if (installerPath == null) {
				this.dispatchError(/* @__PURE__ */ new Error("No update filepath provided, can't quit and install"));
				return false;
			}
			if (path$11.basename(installerPath) === existingBaseName || !/\d+\.\d+\.\d+/.test(existingBaseName)) destination = appImageFile;
			else destination = path$11.join(path$11.dirname(appImageFile), path$11.basename(installerPath));
		...
		async doDownloadUpdate(downloadUpdateOptions) {
			...
			return this.executeDownload({
				...
				task: async (destinationFile, downloadOptions) => {
					const cachedUpdateFilePath = path$10.join(this.downloadedUpdateHelper.cacheDir, CURRENT_MAC_APP_ZIP_FILE_NAME);
		...
		return new Promise((resolve, reject) => {
			...
			(0, child_process_1.execFile)(...preparePowerShellExec(`"Get-AuthenticodeSignature -LiteralPath '${tempUpdateFile}' | ConvertTo-Json -Compress"`, 2e4), (error, stdout, stderr) => {
				...
				try {
					...
					if (data.Status === 0) {
						try {
							const normlaizedUpdateFilePath = path$9.normalize(data.Path);
							const normalizedTempUpdateFile = path$9.normalize(unescapedTempUpdateFile);
		...
		*/
		...
		doInstall(options) {
			...
			const callUsingElevation = () => {
				this.spawnLog(path$8.join(process.resourcesPath, "elevate.exe"), [installerPath].concat(args)).catch((e) => this.dispatchError(e));
			};
		...
		async differentialDownloadWebPackage(downloadUpdateOptions, packageInfo, packagePath, provider) {
			...
			try {
				const downloadOptions = {
					...
					oldFile: path$8.join(this.downloadedUpdateHelper.cacheDir, builder_util_runtime_1.CURRENT_APP_PACKAGE_FILE_NAME),
					logger: this._logger,
					newFile: packagePath,
...
var require_main$1 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function doLoadAutoUpdater() {
		if (process.platform === "win32") _autoUpdater = new (require_NsisUpdater()).NsisUpdater();
			...
			try {
				const identity = path$7.join(process.resourcesPath, "package-type");
...
var require_ipc$1 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var RESTRICTED_TYPES = /* @__PURE__ */ new Set([
		Promise,
		WeakMap,
		WeakSet
	]);
	...
	*/
	function findUp(fileName, cwd) {
		...
		while (true) {
			...
			if (fs$4.existsSync(path$6.join(currentPath, fileName))) return path$6.resolve(path$6.join(currentPath, fileName));
		...
		return Object.assign(transport, {
			...
			archiveLogFn(file) {
				...
				try {
					...
					file.crop(Math.min(quarterOfMaxSize, 262144));
				}
			},
			resolvePathFn(vars) {
...
ipcMain.handle("set-always-on-top", (_event, isTop) => {
...
function createWindow() {
	...
	win.webContents.on("did-finish-load", () => {
		...
		if (!VITE_DEV_SERVER_URL) setTimeout(() => import_main.autoUpdater.checkForUpdatesAndNotify(), 3e3);
	});
...
app.whenReady().then(() => {
	if (process.platform === "win32") app.setAppUserModelId("com.workflowpro.app");
	createWindow();
