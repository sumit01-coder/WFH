    ...
    <div className="p-8 max-w-6xl mx-auto">
      <header className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-slate-500 mt-1">Schedule and join upcoming meetings with your team.</p>
        </div>
        <button 
      ...
      {meetings.length === 0 ? (
        <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center shadow-sm">
          ...
          <p className="text-slate-500">You don't have any scheduled meetings. Click "Schedule Meeting" to create one.</p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-xl font-bold text-slate-900 group-hover:text-blue-600 transition-colors line-clamp-1">{meeting.title}</h3>
                ...
                <div className="space-y-3 mb-6 flex-1">
                  ...
                  {meeting.location && (
                    <div className="flex items-center gap-3 text-slate-500 line-clamp-1">
                ...
                <div className="pt-4 border-t border-slate-100 flex items-center justify-between mt-auto">
                  <div className="text-sm text-slate-500">
                    Organizer: <span className="font-medium text-slate-700">{meeting.organizer?.firstName}</span>
                  </div>
                  {isVideo ? (
                    ...
                    <button className="px-4 py-2 rounded-lg bg-slate-50 text-slate-600 font-semibold hover:bg-slate-100 transition-colors">
                      View Details
                    </button>
                  )}
                </div>
            ...
            >
              ...
              <form onSubmit={handleSchedule} className="p-6 overflow-y-auto">
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      ...
                      <input 
                        ...
                        className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-slate-700"
                      />
                    </div>
                    <div>
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">Location / Video Link</label>
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2 flex items-center justify-between">
                      ...
                      <span className="text-xs font-normal text-slate-500">{selectedParticipants.length} selected</span>
                    </label>
                      ...
                      {users.filter(u => u.id !== user?.id).map(u => (
                        <label key={u.id} className="flex items-center gap-3 p-3 hover:bg-blue-50 cursor-pointer border-b border-slate-100 last:border-0 transition-colors">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs text-slate-500">{u.role?.name}</span>
                          </div>
                        </label>
                      ))}
                      {users.length <= 1 && (
                        <div className="p-4 text-center text-slate-500 text-sm">No other employees found in the company.</div>
                      )}
                    </div>
                  </div>
                </div>
