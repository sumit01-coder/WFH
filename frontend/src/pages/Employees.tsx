    ...
    <div className="p-8 max-w-6xl mx-auto relative">
      <header className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-slate-500 mt-1">View and manage your organization's members.</p>
        </div>
        {canInvite && (
          <button 
              ...
              >
                ...
                <td className="p-4">
                  <span className={`px-3 py-1 border rounded-full text-xs font-bold ${
                    emp.status === 'ACTIVE' ? 'bg-green-50 text-green-600 border-green-200' : 'bg-slate-100 text-slate-600'
                  }`}>
                    {emp.status}
                  </span>
                </td>
              ...
              <div className="p-6">
                {tempPassword ? (
                  <div className="text-center">
                    ...
                    <h4 className="text-xl font-bold text-slate-900 mb-2">User Invited!</h4>
                    <p className="text-slate-500 mb-6 text-sm">Share this temporary password with the user securely. They must change it upon their first login.</p>
                    
                    <div className="bg-slate-100 p-4 rounded-xl flex justify-between items-center border border-slate-200 mb-6 group cursor-pointer hover:bg-slate-200 transition-colors" onClick={copyToClipboard}>
                      <code className="text-lg font-mono text-slate-800 tracking-wider font-bold">{tempPassword}</code>
                      <button className="text-slate-500 group-hover:text-slate-700">
                  ...
                  <form onSubmit={handleInvite} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input required type="text" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                    ...
                    <div>
                      ...
                      <input required type="email" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                    ...
                    <div>
                      ...
                      <select 
                        className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all appearance-none"
