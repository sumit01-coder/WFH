...
import { UserMinus, Plus, X, CheckSquare, Square } from 'lucide-react';
...
const Offboarding = () => {
  ...
  const fetchTasks = async () => {
    try {
      const res = await axios.get(`${API_URL}/offboarding`);
      setTasks(res.data);
    } catch (err) {
      console.error(err);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await axios.post(`${API_URL}/offboarding`, { employeeId, title, description });
  ...
  const toggleTask = async (id: string, currentStatus: boolean) => {
    try {
      await axios.put(`${API_URL}/offboarding/${id}`, { isCompleted: !currentStatus });
      fetchTasks();
    } catch (err: any) {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <header className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-3xl font-bold flex items-center gap-3">
            <UserMinus className="text-blue-500" size={32} />
            Offboarding Checklist
          </h2>
          <p className="text-slate-500 mt-1">Manage exit tasks for departing employees</p>
        </div>
        {canManage && (
          <button 
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
        {tasks.length === 0 ? (
          <div className="text-center text-slate-500 py-8">No offboarding tasks found.</div>
        ) : (
          ...
          <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="flex justify-between items-center p-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                <h3 className="text-xl font-bold text-slate-900 dark:text-white">Assign Offboarding Task</h3>
                <button onClick={() => setIsModalOpen(false)} className="text-slate-400 hover:text-slate-600"><X size={24}/></button>
              </div>
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Departing Employee</label>
                ...
                <div>
                  ...
                  <input type="text" required value={title} onChange={e => setTitle(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g. Return company laptop"/>
                </div>
                <div>
